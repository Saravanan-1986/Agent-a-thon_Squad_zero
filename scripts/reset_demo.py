...
def reset_demo():
    ...
    try:
        # Find demo student by external_id
        demo_student = session.query(Student).filter(Student.external_id == "demo_student").first()


        if demo_student:
            sid = demo_student.id
            print(f"[1/3] Found Demo Student ID={sid}. Cleaning user records...")
